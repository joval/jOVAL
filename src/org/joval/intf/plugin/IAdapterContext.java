...
public interface IAdapterContext {
     ...
     * ConstantVariable, or ExternalVariable which has been provided by a file. An external variable is invariably just a
     * "literal" value (or set of values), as is a ConstantVariable. A LocalVariable can contain an ObjectComponentType (which
     * is a reference to an ObjectType), a LiteralComponentType (which is a constant), or it can contain any of a number of
     * [XYZ]FunctionType objects, which can themselves contain any of these same types of objects, and which cause some
     * function to be applied to each of them.  It therefore makes sense to return a List of VariableValueType objects, each of
     * which contains the resolution of one of these components that makes up the VariableType.
     *
