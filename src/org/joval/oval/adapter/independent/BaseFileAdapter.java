...
public abstract class BaseFileAdapter implements IAdapter {
	...
	for (String path : getPathList(fObj, rc, fs)) {
		...
		if (fObj.isSetFilepath()) {
		    ...
		    String msg = JOVALMsg.getMessage(JOVALMsg.ERROR_TEXTFILECONTENT_SPEC, id);
	...
	try {
		...
		if (fObj.isSetFilename()) {
			...
			for (String fname : fnames) {
				...
				switch(filename.getOperation()) {
				  ...
				  default:
				    throw new OvalException(JOVALMsg.getMessage(JOVALMsg.ERROR_UNSUPPORTED_OPERATION,
		...
		throw new OvalException(JOVALMsg.getMessage(JOVALMsg.ERROR_BAD_FILE_OBJECT, id));
	    }
	...
	if (appIdType == null) {
	    throw new OvalException(JOVALMsg.getMessage(JOVALMsg.ERROR_BAD_PLIST_OBJECT, obj.getId()));
	} else {
