...
function Filter-Processes {
  param(
    [Parameter(Mandatory=$true, Position=0, ValueFromPipeline=$true)][System.Diagnostics.Process]$Process,
    [Parameter(Mandatory=$true)][int]$Bitness
  )
...
namespace jOVAL.Environment58 {
	...
	public static int GetProcessBitness(UInt32 pid) {
	    ProcessAccessFlags flags = ProcessAccessFlags.QueryInformation | ProcessAccessFlags.VMRead;
	    IntPtr hProcess = IntPtr.Zero;
	    hProcess = OpenProcess(flags, false, (int)pid);
	    if (hProcess == IntPtr.Zero) {
	    ...
	    int bitness = 32;
	    try {
		bitness = _GetProcessBitness(hProcess);
	    } finally {
		CloseHandle(hProcess);
	    }
	    return bitness;
...
"@
    ...
    $ErrorActionPreference = "SilentlyContinue"
    $type = [jOVAL.Environment58.Filter]
  ...
  PROCESS {
    if (!($Process -eq $null)) {
      if ([jOVAL.Environment58.Filter]::GetProcessBitness($Process.Id) -eq $Bitness) {
	$Process
      }
    }
  }
