...
public abstract class BaseRegkeyAdapter<T extends ItemType> implements IAdapter, IBatch {
	...
	for (Arguments args : getArguments(obj, rc)) {
	    try {
     ...
     */
    protected final Collection<Arguments> getArguments(ObjectType obj, IRequestContext rc) throws CollectException {
	...
	if (session.supports(view)) {
	    ...
	    //
	    // For each matching top-level hive, create arguments.
	    //
		...
		for (IKey key : getKeys(rObj, hive, rc)) {
			...
			args.add(new Arguments(obj, rItem.it, key, rc));
		    } catch (Exception e) {
    ...
    /**
     * A data structure containing arguments for the getItems abstract subclass method.
     */
    protected class Arguments {
	public ObjectType obj;
	public ItemType base;
	public IKey key;
	public IRequestContext rc;

	Arguments(ObjectType obj, ItemType base, IKey key, IRequestContext rc) {
	    this.obj = obj;
	    this.base = base;
	    this.key = key;
	    this.rc = rc;
     ...
     */
    protected abstract Collection<T> getItems(Arguments args) throws Exception;
