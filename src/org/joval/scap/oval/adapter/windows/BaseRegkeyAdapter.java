 ...
 * @version %I% %G%
 */
public abstract class BaseRegkeyAdapter<T extends ItemType> implements IAdapter, IBatch {
    ...
    /**
     * Determine the sets of tuples of object, item, regkey and request context required for a subclass to retrieve items
     * matching the object specification.
     */
    protected final Collection<IKey> getKeys(ObjectType obj, IRequestContext rc) throws CollectException {
	Collection<IKey> keys = new ArrayList<IKey>();
	...
	if (session.supports(view)) {
	    ...
	    //
	    // For each matching top-level hive, get the keys.
	    //
	    for (IRegistry.Hive hive : hives) {
		keys.addAll(getKeys(rObj, hive, rc));
	    }
	} else {
    ...
    /**
     * Return an ItemType of the appropriate sub-type, with the hive, key and view entities populated according
     * to the specified IKey.
     *
     * @return null if there is a problem generating the item.
     */
    protected ItemType getBaseItem(ObjectType obj, IKey key) throws CollectException {
	String id = obj.getId();
	try {
	    ReflectedRegistryObject rObj = new ReflectedRegistryObject(obj);
	    ...
	    hiveType.setValue(key.getHive().getName());
	    ...
	    switch(getView(rObj.getBehaviors())) {
	      case _32BIT:
	    ...
	    session.getLogger().error(JOVALMsg.getMessage(JOVALMsg.ERROR_EXCEPTION), e);
	    session.getLogger().warn(Message.ERROR_WINREG_KEY, key.toString(), e.getMessage());
	}
	return null;
    }
    ...
    /**
     * Return a list of items to associate with the given ObjectType, based on information gathered from the IKeys.
     *
     * @throws CollectException collection cannot take place and should be halted
     */
    protected abstract Collection<T> getItems(ObjectType obj, Collection<IKey> keys, IRequestContext rc)
