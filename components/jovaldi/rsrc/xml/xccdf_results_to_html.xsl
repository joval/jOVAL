...
ist|remote application|URL)|display (alert|dialog))\\b|^\\s*return\\b"},{cN:"constant",b:"\\b(text item delimiters|current application|missing value)\\b"},{cN:"keyword",b:"\\b(apart from|aside from|instead of|out of|greater than|isn't|(doesn't|does not) (equal|come before|come after|contain)|(greater|less) than( or equal)?|(starts?|ends|begins?) with|contained by|comes (before|after)|a (ref|reference))\\b"},{cN:"property",b:"\\b(POSIX path|(date|time) string|quoted form)\\b"},{cN:"function_start",bWK:true,k:"on",i:"[${=;\\n]",c:[e,d]}].concat(c)}}(hljs);
    ...
    <script langauge="javascript"><![CDATA[ 
      $(document).ready(function(){
        ...
        function generateDiagnosticsView(jData, jView) {
          ...
          for (var i=0; i < arDetailSections.length; i++) {
            ...
            switch (jSection.attr('class').toLowerCase()) {
        ...
        function generateLogicListItems(jItem, jData, oOptions){
          ...
          var sElement = jItem.attr('class').toLowerCase();

          //console.log(sElement+': '+jItem[0].outerHTML);
          ...
          if (bOVALCheck || bSCECheck || bOCILCheck) {
            ...
            jCheckContentRefs = jCheckContentRefs.filter(function(){
            ...
            if (jCheckContentRefs.length > 0) {
              ...
              //console.log(jPossibleCheckContent.length);
              ...
              if (jPossibleCheckContent.length == 0) {
                ...
                arComplexCheck.push('<div class="complex-check" operator="AND" negate="'+ bNegate.toString() +'" result="'+ sRuleResult +'">');
                  jPossibleCheckContent.each(function(){
                    ...
                    arComplexCheck.push('<div class="check" system="'+ sCheckSystem +'"><div class="check-content-ref" name="'+ sCheckContentRefName  +'"></div></div>');
                  });
                arComplexCheck.push('</div>');
                //console.log(arComplexCheck.join(' '));
                //console.log('Complex Check generated.');
          ...
          if (bOVALCheck) {
            ...
            var sClass = jCheckContent.attr('xccdf_class');
        ...
        function generateTestDetail(jTest, jData, bNegate) {
          ...
          jTestResult.children('.tested_item').each(function(){
            ...
            var sItemTagName = jCollectedItem.attr('class').toLowerCase();
            var oItem = {
          ...
          if (bHasStates) {
            jTest.children('.state').each(function(){
              ...
              var sStateTagName = jState.attr('class').toLowerCase();
              var oState = {
                _id: jState.attr('id'),
              ...
              if (oState._is_complex_datatype) {
                ...
                jState.children().each(function(){
                  ...
                  oState[$(this).attr('class').toLowerCase()] = {
          ...
          // TODO: handle large number of states/items       
        ...
        function getChildrenAsObject(jParent, oExtend) {
          ...
          jParent.children().each(function(){
            var jChild = $(this);
            oReturn[jChild.attr('class').toLowerCase()] = jChild.text();
          });
          return oReturn;
        }
